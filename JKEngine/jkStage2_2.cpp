...
#pragma endregion 
	...
	{		//monsterGroup1
		for (int i = 0; i < 10; i++)
		{
			...
			ttr->SetPosition(Vector3(-210 + i * 50, -165, -249));
			AddMonster(_warrior);
		...
		for (int i = 0; i < 3; i++)
		{
			...
			_warrior->Initialize();
			Transform* ttr = _warrior->GetComponent<Transform>();
			ttr->SetPosition(Vector3(-150 + i * 100, -165, -249));
			AddMonster(_warrior);
			_warrior->SetState(GameObject::eState::Paused);
			monsterGroup1.push_back(_warrior);
		}
		for (int i = 0; i < 3; i++)
		{
			...
			_warrior->Initialize();
			Transform* ttr = _warrior->GetComponent<Transform>();
			ttr->SetPosition(Vector3(-210 + i * 200, 165, -249));
			AddMonster(_warrior);
			_warrior->SetState(GameObject::eState::Paused);
			monsterGroup1.push_back(_warrior);
		}
		...
		for (int i = 0; i < 3; i++)
		{
			...
			_warrior->Initialize();
			Transform* ttr = _warrior->GetComponent<Transform>();
			ttr->SetPosition(Vector3(-150 + i * 200, 165, -249));
			AddMonster(_warrior);
			_warrior->SetState(GameObject::eState::Paused);
			monsterGroup3.push_back(_warrior);
		}
