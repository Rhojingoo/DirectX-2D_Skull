...
namespace gui
{
	...
	void Editor::Initialize()
	{
		...
		{
			std::shared_ptr<jk::Mesh> mesh = jk::Resources::Find<jk::Mesh>(L"DebugCircle");
			std::shared_ptr<jk::Material> material = jk::Resources::Find<jk::Material>(L"DebugMaterial");
			mDebugOjbects[(UINT)eColliderType::Circle] = new DebugOjbect();
			mDebugOjbects[(UINT)eColliderType::Circle]->AddComponent<jk::Transform>();
			jk::MeshRenderer* mr = mDebugOjbects[(UINT)eColliderType::Circle]->AddComponent<jk::MeshRenderer>();
			mr->SetMaterial(material);		mr->SetMesh(mesh);
		}
