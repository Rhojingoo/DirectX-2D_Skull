...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    if (ParticleBuffer[DTid.x].active == 0)
    {
        ...
        if (ParticleBuffer[DTid.x].active == 1)
        {
            ...
            float3 randomDirectionR = float3(vRandom.x * 2.0, abs(vRandom.y)* 2.f, vRandom.z * 2.0f - 1.0f);//다이렉션 1번랜덤
            float3 randomDirectionL = float3(vRandom.x * -2.0, abs(vRandom.y) * 2.f, vRandom.z * 2.0f - 1.0f);//다이렉션 1번랜덤
            float3 dir = (noise - 0.5f) * 2.f;// 다이렉션 2번랜덤

     
            if (ParticleBuffer[DTid.x].direction.z == 1.0f)
            {
                randomDirection = randomDirectionR;
            }
            else if (ParticleBuffer[DTid.x].direction.z == -1.0f)
            {
                randomDirection = randomDirectionL;
            }
            ...
            ParticleBuffer[DTid.x].speed = vRandom.w * 1000.f; // 랜덤 속도 설정

            // endTime을 랜덤하게 설정 (예: 1.0초에서 1.5초 사이)
            ParticleBuffer[DTid.x].endTime = lerp(1.f, 1.5f, vRandom.w);
        }
    }
    else
    {
